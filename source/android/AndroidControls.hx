...
class AndroidControls extends FlxSpriteGroup {
	...
	public function new() {
		...
		switch (mode){
			...
			case DUO:
				initControler(3);
			case HITBOX:
		    if(EnginePreferences.data.hitboxmode != 'New'){
				initControler(4);
		    }else{
		    initControler(5);
		    }
	...
	function initControler(vpadMode:Int) {
		switch (vpadMode){
			case 0:
				vpad = new FlxVirtualPad(RIGHT_FULL, NONE, 0.75, EnginePreferences.data.antialiasing);	
				add(vpad);						
			case 1:
				vpad = new FlxVirtualPad(FULL, NONE, 0.75, EnginePreferences.data.antialiasing);
				add(vpad);			
			case 2:
				vpad = new FlxVirtualPad(FULL, NONE, 0.75, EnginePreferences.data.antialiasing);
				...
				add(vpad);	
			case 3:
				vpad = new FlxVirtualPad(DUO, NONE, 0.75, EnginePreferences.data.antialiasing);
				add(vpad);		
			case 4:
				hbox = new FlxHitbox(0.75, EnginePreferences.data.antialiasing);
				add(hbox);
			case 5:
