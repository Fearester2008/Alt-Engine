...
class FlxNewHitbox extends FlxSpriteGroup
{
	...
	{
		super();
        if (EnginePreferences.data.hitboxExtend != true){
            ...
            if (EnginePreferences.data.hitboxLocation == 'Bottom'){
		    ...
		    else if (EnginePreferences.data.hitboxLocation == 'Top'){
	...
	private function createHint(X:Float, Y:Float, Width:Int, Height:Int, Color:Int = 0xFFFFFF):FlxButton
	{
		...
		{
			if (hint.alpha != EnginePreferences.data.hitboxalpha)
				hint.alpha = EnginePreferences.data.hitboxalpha;
		}
