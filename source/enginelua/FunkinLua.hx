...
class FunkinLua {
	...
	public function new(script:String) {
		...
		Lua_helper.add_callback(lua, "parseJson", function(jsonStr:String, varName:String) {
			var json = Paths.modFolders('jsons/' + jsonStr + '.json');
			var foundJson:Bool;
		...
		Lua_helper.add_callback(lua, "parseFile", function(fileStr:String, ext:String, varName:String) {
			var file = Paths.modFolders('custom_types/' + fileStr + '.' + ext);
			var foundFile:Bool;
			...
			trace(Assets.exists(file));
				...
				if (FileSystem.exists(file)) {
					...
					luaTrace('parseFile: Invalid ' + ext + ' file path!', false, false, FlxColor.RED);
					foundFile = false;
					return;	
				}
			#else
			...
			if (foundFile) {
				var parsedFile = haxe.Json.parse(File.getContent(file));				
				PlayState.instance.variables.set(varName, parsedFile);
			}
		});
