...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		logoBl.antialiasing = EnginePreferences.data.antialiasing;
		...
		if(EnginePreferences.data.shaders) swagShader = new ColorSwap();
		...
		if (animFrames.length > 0) {
			...
			titleText.animation.addByPrefix('press', EnginePreferences.data.flashing ? "ENTER PRESSED" : "ENTER FREEZE", 24);
		}
		else {
			newTitle = false;
		...
		ngSpr.antialiasing = EnginePreferences.data.antialiasing;
	...
	{
		AppUtil.setAppData(AppController.appName, AppController.altEngineVersion + AppController.stage, "Welcome To Alt Engine! In Title Menu.");
		...
		if (initialized && !transitioning && skippedIntro)
		{
			...
			if(pressedEnter)
			{
				...
				FlxG.camera.flash(EnginePreferences.data.flashing ? FlxColor.BLACK : 0x4CFFFFFF, 1);
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 4:
					createCoolText(['Fearester2008', 'Stefan2008', 'Rom4chek', 'Joalor64', 'Daniel', 'TheLeerName', 'Sussy Sam'], -40);
