...
class FreeplayState extends MusicBeatState
{
	...
	public var camInstruction:FlxCamera;
	...
	public var bottomBGLoad:FlxSprite;
	...
	override function create()
	{
		...
		bottomBG = new FlxSprite(0, FlxG.height).makeGraphic(FlxG.width - 100, FlxG.height - 100, 0xFF000000);
		bottomBG.alpha = 0.6;
		bottomBG.screenCenter(X);
		add(bottomBG);

		bottomBGLoad = new FlxSprite(0, FlxG.height - 26).makeGraphic(FlxG.width, 26, 0xFF00AD1D);
		bottomBGLoad.alpha = 0.6;
		bottomBGLoad.scale.set(0, 1);
		add(bottomBGLoad);

		var size:Int = 16;
		bottomText = new FlxText(0, 0, FlxG.width - 100, '', size);
		...
		exitButton = new Button(0, 5, 60, 60, "X", function(){		
		visibleText = false;
		if(!visibleText)
		{
			FlxTween.tween(bottomBG, {y: FlxG.height}, 0.6, {ease:FlxEase.backIn});
		}
		});
		add(exitButton);
		...
		bottomBG.cameras = [camInstruction];
		bottomBGLoad.cameras = [camInstruction];
		bottomText.cameras = [camInstruction];
		exitButton.cameras = [camInstruction];
	...
	override function update(elapsed:Float)
	{
		...
		var ratingSplit:Array<String> = Std.string(MathUtil.truncateFloat(lerpRating * 100, 2)).split('.');
		...
		bottomText.y = bottomBG.y + 20;
		exitButton.y = bottomBG.y;
		exitButton.x = bottomBG.width - 10;
		...
		{
			notifyBox.updateText(null, "Loaded Successfully: " + songs[curSelected].songName);
	...
	public function loadSong(curSelected:Int)
	{
				...
				{
					music = Paths.inst(PlayState.SONG.song, PlayState.SONG.songPostfix);
				}
				else
				{
					music = Paths.inst(PlayState.SONG.song);
				}
				inst = new FlxSound();
