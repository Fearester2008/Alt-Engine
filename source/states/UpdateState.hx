...
class UpdateState extends MusicBeatState
{
	...
	public function new(baseURL:String = "http://raw.githubusercontent.com/Fearester2008/Alt-Engine/main/", fileList:Array<String>) 
	{
		super();
	...
	function doFile() {
		...
		downloadStream.addEventListener(ProgressEvent.PROGRESS, function(e) {
			var label1 = '(${totalFiles - fileList.length}/${totalFiles})';
			var label2 = '(${CoolUtil.getInterval(Std.int(e.bytesLoaded))} / ${CoolUtil.getInterval(Std.int(e.bytesTotal))})';
			
			var ll = CoolUtil.getInterval(Std.int((e.bytesLoaded - oldBytesLoaded) / (t - oldTime)));
	...
	public function applyUpdate() {
		...
		File.copy('AltEngine.exe', 'temp.exe');
