...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		magenta.antialiasing = EnginePreferences.data.antialiasing;
		...
		menuItem.antialiasing = EnginePreferences.data.antialiasing;
		...
		if (leDate.getDay() == 5 && leDate.getHours() >= 18)
			backend.Achievements.unlockAchievement('friday_night_play');

		#if MODS_ALLOWED
		backend.Achievements.loadAchievements();
		#end
		#end
		...
		if (!selectedSomethin)
		{
			...
			if (controls.ACCEPT)
			{
				FlxG.sound.play(Paths.sound('confirmMenu'));
					...
					if (EnginePreferences.data.flashing)
