...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		eventIcon.antialiasing = EnginePreferences.data.antialiasing;
		...
		dummyArrow.antialiasing = EnginePreferences.data.antialiasing;
		add(dummyArrow);
	...
	var UI_songPostfix:FlxUIInputText;

	var stageDropDown:FlxUIDropDownMenu;
	var sliderRate:FlxUISlider;
	function addSongUI():Void
	{
		UI_songTitle = new FlxUIInputText(10, 25, 70, _song.song, 8);
		UI_songTitle.focusGained = () -> FlxG.stage.window.textInputEnabled = true;
		blockPressWhileTypingOn.push(UI_songTitle);
		
		UI_songComposer = new FlxUIInputText(10, 60 , 70, _song.composer, 8);
		UI_songComposer.focusGained = () -> FlxG.stage.window.textInputEnabled = true;
		blockPressWhileTypingOn.push(UI_songComposer);

		UI_songPostfix = new FlxUIInputText(10, 95 , 70, _song.songPostfix, 8);
		UI_songPostfix.focusGained = () -> FlxG.stage.window.textInputEnabled = true;
		blockPressWhileTypingOn.push(UI_songPostfix);

		var check_voices = new FlxUICheckBox(10, 120, null, null, "Has voice track", 100);
		...
		var stepperBPM:FlxUINumericStepper = new FlxUINumericStepper(10, 160, 1, 1, 1, 400, 3);
		...
		tab_group_song.add(new FlxText(UI_songTitle.x - 2, UI_songTitle.y - 15, 0, 'Song Name:'));

		tab_group_song.add(UI_songComposer);
		tab_group_song.add(new FlxText(UI_songComposer.x - 2, UI_songComposer.y - 15, 0, 'Composer Name:'));

		tab_group_song.add(UI_songPostfix);
		tab_group_song.add(new FlxText(UI_songPostfix.x - 2, UI_songPostfix.y - 15, 0, 'Song Postfix Name:'));
	...
	function addChartingUI() {
		...
		check_mute_inst = new FlxUICheckBox(10, 290, null, null, "Mute Instrumental (in editor)", 100);
	...
	function loadSong():Void
	{
		...
		{
			var playerVocals;
			playerVocals = Paths.voices(currentSongName, (_song.songPostfix != null) ? _song.songPostfix + '-player' : 'player');
		...
		{
			var oppVocals;
			oppVocals = Paths.voices(currentSongName, (_song.songPostfix != null) ? _song.songPostfix + '-opponent' : 'opponent');
	...
	function generateSong() {
		...
		FlxG.sound.playMusic(Paths.inst(currentSongName, _song.songPostfix), 0.6/*, false*/);
	...
	{
		AppUtil.setAppData(AppController.appName, AppController.altEngineVersion + AppController.stage, "In Chart Editor: " + _song.song);
		...
		_song.songPostfix = (UI_songPostfix.text.length > 0) ? UI_songPostfix.text : null;
		...
		if (!blockInput)
		{
			...
			if(!vortex){
				if (FlxG.keys.justPressed.UP || FlxG.keys.justPressed.DOWN)
				{
					...
					{
						var fuck:Float = CoolUtil.quantize(beat, snap) - increase;
						smTime = Conductor.beatToSeconds(fuck);
					}else{
						var fuck:Float = CoolUtil.quantize(beat, snap) + increase; //(Math.floor((beat+snap) / snap) * snap);
						smTime = Conductor.beatToSeconds(fuck);
					}
					FlxTween.tween(FlxG.sound.music, {time:smTime}, 0.1, {ease:FlxEase.circOut});
				}
			}
	...
	/*function loadAudioBuffer() {
		...
		audioBuffers[0] = null;
		#if MODS_ALLOWED
		if(FileSystem.exists(Paths.modFolders('songs/' + currentSongName + '/FlxG.sound.music.ogg'))) {
			audioBuffers[0] = AudioBuffer.fromFile(Paths.modFolders('songs/' + currentSongName + '/FlxG.sound.music.ogg'));
			//trace('Custom vocals found');
		}
		else { #end
			var leVocals:String = Paths.getPath(currentSongName + '/FlxG.sound.music.' + Paths.SOUND_EXT, SOUND, 'songs');
