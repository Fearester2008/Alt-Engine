...
class EditorPlayState extends MusicBeatSubstate
{
	...
	public function new(playbackRate:Float)
	{
		...
		Conductor.safeZoneOffset = (EnginePreferences.data.safeFrames / 60) * 1000 * playbackRate;
		...
		cachePopUpScore();
		if(EnginePreferences.data.hitsoundVolume > 0) Paths.sound('hitsound');
	...
	{
		AppUtil.setAppData(AppController.appName, AppController.altEngineVersion + AppController.stage, "In Song Test - " + PlayState.SONG.song);
		...
		var time:Float = CoolUtil.floorDecimal((Conductor.songPosition - EnginePreferences.data.noteOffset) / 1000, 1);
	...
	{
		if (PlayState.SONG.needsVoices && FlxG.sound.music.time >= -EnginePreferences.data.noteOffset)
			{
	...
	function generateSong(dataPath:String)
	{
		...
		var songSpeedType:String = EnginePreferences.getGameplaySetting('scrolltype');
		switch(songSpeedType)
		{
			case "multiplicative":
				songSpeed = PlayState.SONG.speed * EnginePreferences.getGameplaySetting('scrollspeed');
			case "constant":
	...
	{
		var strumLineX:Float = EnginePreferences.data.middleScroll ? PlayState.STRUM_X_MIDDLESCROLL : PlayState.STRUM_X;
	...
	{
		if(EnginePreferences.data.noteOffset <= 0) {
			endSong();
		} else {
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		for (i in seperatedScore)
		{
			...
			numScore.x = coolText.x + (43 * daLoop) - 90 + EnginePreferences.data.comboOffset[2];
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (EnginePreferences.data.hitsoundVolume > 0 && !note.hitsoundDisabled)
				FlxG.sound.play(Paths.sound('hitsound'), EnginePreferences.data.hitsoundVolume);
