...
class EditorPlayState extends MusicBeatSubstate
{
	...
	function generateSong(dataPath:String)
	{
		...
		try
		{
			if (songData.needsVoices)
			{
				...
				if(songData.songPostfix != null)
					playerVocals = Paths.voices(songData.song, (characterData.vocalsP1 == null || characterData.vocalsP1.length < 1) ? songData.songPostfix + '-player' : songData.songPostfix + '-' + characterData.vocalsP1);
				else
					playerVocals = Paths.voices(songData.song, (characterData.vocalsP1 == null || characterData.vocalsP1.length < 1) ? 'player' : characterData.vocalsP1);

				if(vocals != null) vocals.loadEmbedded(playerVocals != null ? playerVocals : Paths.voices(songData.song));
				
				var oppVocals;
				if(songData.songPostfix != null)
					oppVocals = Paths.voices(songData.song, (characterData.vocalsP2 == null || characterData.vocalsP2.length < 1) ? songData.songPostfix + '-opponent' : songData.songPostfix + '-' + characterData.vocalsP2);
				else
					oppVocals = Paths.voices(songData.song, (characterData.vocalsP2 == null || characterData.vocalsP2.length < 1) ? 'opponent' : characterData.vocalsP2);
		...
		inst = new FlxSound().loadEmbedded(Paths.inst(songData.song, songData.songPostfix));
	...
	function updateJsonData():Void
		{
			for (i in 1...3)
			{
				var data:CharacterFile = loadCharacterFile(Reflect.field(ChartingState._song, 'player$i'));
