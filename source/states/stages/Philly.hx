...
class Philly extends BaseStage
{
	...
	override function eventPushed(event:objects.Note.EventNote)
	{
		switch(event.event)
		{
			case "Philly Glow":
				...
				if(!EnginePreferences.data.flashing) phillyGlowGradient.intendedAlpha = 0.7;
			...
			case "Philly Glow":
				...
				switch(lightId)
				{
					case 0:
						...
						{
							doFlash();
							if(EnginePreferences.data.camZooms)
							{
					...
					case 1: //turn on
						...
						else if(EnginePreferences.data.flashing)
						{
						...
						if(!EnginePreferences.data.flashing) charColor.saturation *= 0.5;
	...
	function doFlash()
	{
		...
		if(!EnginePreferences.data.flashing) color.alphaFloat = 0.5;
