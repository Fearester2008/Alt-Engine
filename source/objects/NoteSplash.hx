...
class NoteSplash extends FlxSprite
{
	...
	public function setupNoteSplash(x:Float, y:Float, direction:Int = 0, ?note:Note = null) {
		...
		alpha = EnginePreferences.data.splashAlpha;
		if(note != null) alpha = note.noteSplashData.a;
		rgbShader.copyValues(tempShader);

		if(note != null) antialiasing = note.noteSplashData.antialiasing;
		if(PlayState.isPixelStage || !EnginePreferences.data.antialiasing) antialiasing = false;
	...
	{
		var skin:String = '';
		if(EnginePreferences.data.splashSkin != EnginePreferences.defaultData.splashSkin)
			skin = '-' + EnginePreferences.data.splashSkin.trim().toLowerCase().replace(' ', '_');
		return skin;
	}
