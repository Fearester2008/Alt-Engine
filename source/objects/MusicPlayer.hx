...
class MusicPlayer extends MusicBeatState
{
	...
	public var visibleText:Bool = false;
	...
	public function new(instance:FreeplayState)
	{
		...
		addBehindStuff(instance.bottomText, instance.bg);
		instance.bg.y = FlxG.height;
		FlxTween.tween(instance.bg, {y: 0}, 1.5, {ease: FlxEase.sineInOut});
		...
		timePositionTxt.setFormat(Paths.font("FrozenCrystal.otf"), 24, FlxColor.WHITE, LEFT);
		add(timePositionTxt);

		timeLengthTxt = new FlxText(FlxG.width - 75, songBG.y, 0, "test", 32);
		timeLengthTxt.setFormat(Paths.font("FrozenCrystal.otf"), 24, FlxColor.WHITE, RIGHT);
		add(timeLengthTxt);
	...
	function buttonsList()
	{
		...
		var button = new Button(FlxG.width - 130, 5, 60, 60, "i", function(){
		
		visibleText = true;
		if(visibleText)
		{
			FlxTween.tween(instance.bottomBG, {y: 40}, 0.6, {ease:FlxEase.backOut});
		}
		});
		add(button);
	}

	function addBehindStuff(initStuff:FlxBasic, obj:FlxBasic)
	{
		insert(members.indexOf(initStuff), obj);
	}

	function bop(camGameZoom:Float = 0.015)
	{
		if (instance.bg.scale.x < 1.1 && ClientPrefs.data.camZooms)
		{
		instance.bg.scale.x += camGameZoom;
		instance.bg.scale.y += camGameZoom;
		}
	}

	function iconBop(iconBop = 1.2)
	{
...
*/
	...
	{
		var mult = FlxMath.lerp(1, instance.bg.scale.x, Math.exp(-elapsed * 3.125 * playbackRate));
		instance.bg.scale.set(mult, mult);

		var mult = FlxMath.lerp(1, instance.iconOpponentGroup.members[FreeplayState.curSelected].scale.x, Math.exp(-elapsed * 3.125 * playbackRate));
		instance.iconOpponentGroup.members[FreeplayState.curSelected].scale.set(mult, mult);

		var mult = FlxMath.lerp(1, instance.iconBoyfriendGroup.members[FreeplayState.curSelected].scale.x, Math.exp(-elapsed * 3.125 * playbackRate));
	...
	function updateAmplitude(sound:FlxSound) {
		...
		while (index < length) {
			...
			if (rows - prevRows >= samplesPerRow) {
				...
				curAmplitude = (((rmax - rmin) * 2 * 50) / 48.6);
