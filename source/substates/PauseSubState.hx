...
class PauseSubState extends MusicBeatSubstate
{
	...
	override function create()
	{
		...
		PlayState.instance.camHUD.filters = [blur];
		}
		...
		if (controls.ACCEPT && (cantUnpause <= 0 || !controls.controllerMode))
		{
			...
			switch (daSelected)
			{
				...
				case 'Options':
					...
					FlxG.sound.playMusic(Paths.music(Paths.inst(PlayState.SONG.song, PlayState.SONG.songPostfix)), pauseMusic.volume);
	...
	{
		skipTimeText.text = TimeUtil.formatTime(Math.max(0, Math.floor(curTime / 1000)), false) + ' / ' + FlxStringUtil.formatTime(Math.max(0, Math.floor(FlxG.sound.music.length / 1000)), false);
	}
}
