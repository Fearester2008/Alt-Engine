...
class NoteOffsetState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		comboTxt = new FlxSprite().loadGraphic(Paths.image('combo'));
		comboTxt.cameras = [camHUD];
		comboTxt.setGraphicSize(Std.int(comboTxt.width * 0.7));
	...
	override public function update(elapsed:Float)
	{
		...
		if(onComboMenu)
		{
			...
			if (FlxG.mouse.justPressed)
			{
				...
				else if (startMousePos.x - comboTxt.x >= 0 && startMousePos.x - comboTxt.x <= comboTxt.width &&
	...
	function repositionCombo()
	{
		...
		comboTxt.x = coolText.x + 40 + ClientPrefs.comboOffset[4];
		comboTxt.y -= 60 + ClientPrefs.comboOffset[5];
