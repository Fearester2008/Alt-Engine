...
class NoteOffsetState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		rating.antialiasing = EnginePreferences.data.antialiasing;
		...
		for (i in seperatedScore)
		{
			...
			numScore.antialiasing = EnginePreferences.data.antialiasing;
		...
		barPercent = EnginePreferences.data.noteOffset;
		updateNoteDelay();
	...
	{
		AppUtil.setAppData(AppController.appName, AppController.altEngineVersion + AppController.stage, "In Offset Menu ");
		...
		if(onComboMenu)
		{
				...
				if(controlArray.contains(true))
				{
					for (i in 0...controlArray.length)
					{
						if(controlArray[i])
						{
							...
							{
								case 0:
									EnginePreferences.data.comboOffset[0] -= addNum;
								case 1:
									EnginePreferences.data.comboOffset[0] += addNum;
								case 2:
									EnginePreferences.data.comboOffset[1] += addNum;
								case 3:
									EnginePreferences.data.comboOffset[1] -= addNum;
								case 4:
									EnginePreferences.data.comboOffset[2] -= addNum;
								case 5:
									EnginePreferences.data.comboOffset[2] += addNum;
								case 6:
									EnginePreferences.data.comboOffset[3] += addNum;
								case 7:
									EnginePreferences.data.comboOffset[3] -= addNum;
							}
						}
			...
			if (FlxG.mouse.justPressed || gamepadPressed)
			{
					...
					startMousePos.y - comboNums.y >= 0 && startMousePos.y - comboNums.y <= comboNums.height)
				{
					...
					startComboOffset.x = EnginePreferences.data.comboOffset[2];
					startComboOffset.y = EnginePreferences.data.comboOffset[3];
					//trace('yo bro');
				}
						 ...
						 startMousePos.y - rating.y >= 0 && startMousePos.y - rating.y <= rating.height)
				{
					...
					startComboOffset.x = EnginePreferences.data.comboOffset[0];
					startComboOffset.y = EnginePreferences.data.comboOffset[1];
					//trace('heya');
				}
			}
			...
			if(holdingObjectType != null)
			{
				if(FlxG.mouse.justMoved || analogMoved)
				{
					...
					EnginePreferences.data.comboOffset[addNum + 0] = Math.round((mousePos.x - startMousePos.x) + startComboOffset.x);
					EnginePreferences.data.comboOffset[addNum + 1] = -Math.round((mousePos.y - startMousePos.y) - startComboOffset.y);
					repositionCombo();
				}
			}
			...
			{
				for (i in 0...EnginePreferences.data.comboOffset.length)
				{
					EnginePreferences.data.comboOffset[i] = 0;
				}
				repositionCombo();
			}
		}
		else
		{
			...
			{
				barPercent = Math.max(delayMin, Math.min(EnginePreferences.data.noteOffset - 1, delayMax));
				updateNoteDelay();
			}
		...
		if(controls.BACK)
		{
			...
			{
				if(EnginePreferences.data.pauseMusic != 'None')
					FlxG.sound.playMusic(Paths.music(Paths.formatToSongPath(EnginePreferences.data.pauseMusic)));
				else
	...
	{
		rating.screenCenter();
		rating.x = coolText.x - 40 + EnginePreferences.data.comboOffset[0];
		rating.y -= 60 + EnginePreferences.data.comboOffset[1];

		comboNums.screenCenter();
		comboNums.x = coolText.x - 90 + EnginePreferences.data.comboOffset[2];
		comboNums.y += 80 - EnginePreferences.data.comboOffset[3];
		reloadTexts();
	}
	...
	function reloadTexts()
	{
		for (i in 0...dumbTexts.length)
		{
			switch(i)
			{
				...
				case 1: dumbTexts.members[i].text = '[' + EnginePreferences.data.comboOffset[0] + ', ' + EnginePreferences.data.comboOffset[1] + ']';
				case 2: dumbTexts.members[i].text = 'Numbers Offset:';
				case 3: dumbTexts.members[i].text = '[' + EnginePreferences.data.comboOffset[2] + ', ' + EnginePreferences.data.comboOffset[3] + ']';
			}
		}
	}
	...
	{
		EnginePreferences.data.noteOffset = Math.round(barPercent);
