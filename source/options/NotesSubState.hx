...
class NotesSubState extends MusicBeatSubstate
{
	...
	AppUtil.setAppData(AppController.appName, AppController.altEngineVersion + AppController.stage, "In Notes Settings");
		...
		else if(controls.RESET #if android || MusicBeatSubstate._virtualpad.buttonC.justPressed || MusicBeatSubstate._virtualpad.buttonE.justPressed #end && hexTypeNum < 0)
		{
			if(FlxG.keys.pressed.SHIFT || FlxG.gamepads.anyJustPressed(LEFT_SHOULDER) #if android || MusicBeatSubstate._virtualpad.buttonE.justPressed #end)
			{
				for (i in 0...3)
				{
					...
					var color:FlxColor = !onPixel ? EnginePreferences.defaultData.arrowRGB[curSelectedNote][i] :
													EnginePreferences.defaultData.arrowRGBPixel[curSelectedNote][i];
					switch(i)
					{
						case 0:
			...
			setShaderColor(!onPixel ? EnginePreferences.defaultData.arrowRGB[curSelectedNote][curSelectedMode] : EnginePreferences.defaultData.arrowRGBPixel[curSelectedNote][curSelectedMode]);
			FlxG.sound.play(Paths.sound('cancelMenu'), 0.6);
			updateColors();
		}
	}
	...
	{
		dataArray = !onPixel ? EnginePreferences.data.arrowRGB : EnginePreferences.data.arrowRGBPixel;
		...
		skinNote.antialiasing = EnginePreferences.data.antialiasing;
		...
		for (i in 0...3)
		{
			...
			newNote.antialiasing = EnginePreferences.data.antialiasing;
