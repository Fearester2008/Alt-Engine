...
class ControlsSubState extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		controllerSpr.antialiasing = EnginePreferences.data.antialiasing;
	...
	function addKeyText(text:Alphabet, option:Array<Dynamic>, id:Int)
	{
		for (n in 0...2)
		{
			...
			if(onKeyboardMode)
			{
				var savKey:Array<Null<FlxKey>> = EnginePreferences.keyBinds.get(option[2]);
				key = InputFormatter.getKeyName((savKey[n] != null) ? savKey[n] : NONE);
			}
			else
			{
				var savKey:Array<Null<FlxGamepadInputID>> = EnginePreferences.gamepadBinds.get(option[2]);
	...
	{
		AppUtil.setAppData(AppController.appName, AppController.altEngineVersion + AppController.stage, "In Controls Settings Menu");
		...
		if(!binding)
		{
			...
			if(FlxG.keys.justPressed.ENTER || FlxG.gamepads.anyJustPressed(START) || FlxG.gamepads.anyJustPressed(A))
			{
				...
				{
					// Reset to Default
					EnginePreferences.resetKeys(!onKeyboardMode);
		...
		else
		{
			...
			else if (FlxG.keys.pressed.BACKSPACE || FlxG.gamepads.anyPressed(BACK))
			{
				...
				if(holdingEsc > 0.5)
				{
					EnginePreferences.keyBinds.get(curOption[2])[altNum] = NONE;
					EnginePreferences.clearInvalidKeys(curOption[2]);
			...
			else
			{
				...
				var curKeys:Array<FlxKey> = EnginePreferences.keyBinds.get(curOption[2]);
				var curButtons:Array<FlxGamepadInputID> = EnginePreferences.gamepadBinds.get(curOption[2]);
				...
				if(changed)
				{
					...
					EnginePreferences.clearInvalidKeys(option);
					for (n in 0...2)
					{
						var key:String = null;
						if(onKeyboardMode)
						{
							var savKey:Array<Null<FlxKey>> = EnginePreferences.keyBinds.get(option);
							key = InputFormatter.getKeyName(savKey[n] != null ? savKey[n] : NONE);
						}
						else
						{
							var savKey:Array<Null<FlxGamepadInputID>> = EnginePreferences.gamepadBinds.get(option);
							key = InputFormatter.getGamepadName(savKey[n] != null ? savKey[n] : NONE);
						}
