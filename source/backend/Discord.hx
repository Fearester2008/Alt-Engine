...
class DiscordClient
{
	...
	private static final _defaultID:String = "1151492369959620689";
	public static var clientID(default, set):String = _defaultID;
	private static var presence:DiscordRichPresence = DiscordRichPresence.create();

	public static function check()
	{
		if(EnginePreferences.data.discordRPC) initialize();
		else if(isInitialized) shutdown();
	}
	
	public static function prepare()
	{
		if (!isInitialized && EnginePreferences.data.discordRPC)
			initialize();
	...
	public static function changePresence(?details:String = 'In the Menus', ?state:Null<String>, ?smallImageKey : String, ?hasStartTimestamp : Bool, ?endTimestamp: Float)
	{
		...
		presence.largeImageText = "Engine Version: " + AppController.altEngineVersion;
