...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		{
		case 'Alt Old':
		timeBar = new FlxBar(FlxG.width - 790, (!ClientPrefs.downScroll) ? 30 : FlxG.height - 30, LEFT_TO_RIGHT, 790, 20, this, 'songPercent', 0, 1);
		timeBar.createImageBar(Paths.image('timebars/old/timeBar-old'), Paths.image('timebars/old/timeBarProgress-old'), FlxColor.RED, FlxColor.GREEN);
		timeBar.color = FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]);
		case 'Alt New':
		timeBar = new FlxBar(FlxG.width - 960, (!ClientPrefs.downScroll) ? 30 : FlxG.height - 30, LEFT_TO_RIGHT, 960, 20, this, 'songPercent', 0, 1);
		...
		{
			if(ClientPrefs.timeBarVisual != 'Psych')
			{
			timeTxt.size = 20;
			timeTxt.y -= 2;
			}
		...
		scoreTextBG = new FlxSprite(0,(FlxG.height * 0.89) + 36).makeGraphic(FlxG.width - 200, 25, FlxColor.BLACK);
		scoreTextBG.screenCenter(X);
		scoreTextBG.visible = !ClientPrefs.hideHud || !cpuControlled;
		scoreTextBG.alpha = 0.6;
		add(scoreTextBG);

		scoreTxt = new FlxText(0, (FlxG.height * 0.89) + 36 , FlxG.width, "", 22);
    ...
    function moveSText(?isReady:Bool = true)
		{
			if(isReady == true)
			{
				...
				{
				scoreTextTween = FlxTween.tween(scoreTxt, {y: healthBarBG.y - 36 , alpha: 1}, 1, {
					ease: FlxEase.linear,
					onComplete: function(twn:FlxTween)
					{
						scoreTextTween = FlxTween.tween(scoreTxt, {y: healthBarBG.y - 190 , alpha: 0}, (Conductor.crochet / 1000), {
							ease: FlxEase.linear,
							onComplete: function(twn:FlxTween)
							{
								scoreTextTween = null;
				...
				else
				{
					scoreTextTween = FlxTween.tween(scoreTxt, {y: healthBarBG.y + 36 , alpha: 1}, 1, {
						...
						onComplete: function(twn:FlxTween)
						{
							scoreTextTween = FlxTween.tween(scoreTxt, {y: healthBar.y + 95 , alpha: 0}, (Conductor.crochet / 1000), {
								ease: FlxEase.linear,
								onComplete: function(twn:FlxTween)
								{
									scoreTextTween = null;
			...
			function moveSBG(?isGo:Bool = true)
				{
					if(isGo == true)
					{
						...
						if(ClientPrefs.downScroll)
						{
						scorextTween = FlxTween.tween(scoreTextBG, {y: healthBarBG.y - 36 , alpha: 0.6}, 1, {
							ease: FlxEase.linear,
							onComplete: function(twn:FlxTween)
							{
								scorextTween = FlxTween.tween(scoreTextBG, {y: healthBarBG.y - 190 , alpha: 0}, (Conductor.crochet / 1000), {
									ease: FlxEase.linear,
									onComplete: function(twn:FlxTween)
									{
										scorextTween = null;
						...
						else
						{
							scorextTween = FlxTween.tween(scoreTextBG, {y: healthBarBG.y + 36 , alpha: 0.6}, 1, {
								...
								onComplete: function(twn:FlxTween)
								{
									scorextTween = FlxTween.tween(scoreTextBG, {y: healthBar.y + 95 , alpha: 0}, (Conductor.crochet / 1000), {
										ease: FlxEase.linear,
										onComplete: function(twn:FlxTween)
										{
											scorextTween = null;
	...
	public function endSong():Void
	{
		...
		for(hudDelete in [#if android androidc, #end timeBar, timeTxt, songWatermark, judgementCounter, waterBg, musicianWatermark])
		hudDelete.visible = false;
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					{
						openSubState(new ResultsScreen([sicks, goods, bads, shits], campaignScore, songMisses,
			...
			else
			{
				...
				if(ClientPrefs.results)
				{
					openSubState(new ResultsScreen([sicks, goods, bads, shits], songScore, songMisses,
						HelperFunctions.truncateFloat(ratingPercent * 100, 2)));				}
				else
				{
	...
	override function beatHit()
	{
		...
		if (camZooming && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms && curBeat % 1 == 0 && ClientPrefs.beatMode == 'Both camera' && ClientPrefs.beatType == '1/2')
		{
			FlxG.camera.zoom += 0.012 * ClientPrefs.camSpeed;
			camHUD.zoom += 0.016 * ClientPrefs.camSpeed;
			camNOTES.zoom += 0.023 * ClientPrefs.camSpeed;

		}
		if (camZooming && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms && curBeat % 1 == 0 && ClientPrefs.beatMode == 'HUD camera' && ClientPrefs.beatType == '1/2')
		{
			camHUD.zoom += 0.016 * ClientPrefs.camSpeed;
			camNOTES.zoom += 0.023 * ClientPrefs.camSpeed;
		}
		if (camZooming && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms && curBeat % 1 == 0 && ClientPrefs.beatMode == 'Game camera' && ClientPrefs.beatType == '1/2')
		{
			FlxG.camera.zoom += 0.012 * ClientPrefs.camSpeed;
		}
		if (camZooming && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms && curBeat % 2 == 0 && ClientPrefs.beatMode == 'Both camera' && ClientPrefs.beatType == '1/4')
		{
			FlxG.camera.zoom += 0.012 * ClientPrefs.camSpeed;
			camHUD.zoom += 0.016 * ClientPrefs.camSpeed;
			camNOTES.zoom += 0.023 * ClientPrefs.camSpeed;
		...
		if (camZooming && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms && curBeat % 2 == 0 && ClientPrefs.beatMode == 'HUD camera' && ClientPrefs.beatType == '1/4')
		{
			camHUD.zoom += 0.016 * ClientPrefs.camSpeed;
			camNOTES.zoom += 0.023 * ClientPrefs.camSpeed;

		}
		if (camZooming && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms && curBeat % 2 == 0 && ClientPrefs.beatMode == 'Game camera' && ClientPrefs.beatType == '1/4')
		{
			FlxG.camera.zoom += 0.012 * ClientPrefs.camSpeed;
		}
		if (camZooming && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms && curBeat % 4 == 0 && ClientPrefs.beatMode == 'Both camera' && ClientPrefs.beatType == '1/16')
		{
			FlxG.camera.zoom += 0.012 * ClientPrefs.camSpeed;
			camHUD.zoom += 0.016 * ClientPrefs.camSpeed;
			camNOTES.zoom += 0.023 * ClientPrefs.camSpeed;
		}
		if (camZooming && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms && curBeat % 4 == 0 && ClientPrefs.beatMode == 'HUD camera' && ClientPrefs.beatType == '1/16')
		{
			camHUD.zoom += 0.016 * ClientPrefs.camSpeed;
			camNOTES.zoom += 0.023 * ClientPrefs.camSpeed;

		}
		if (camZooming && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms && curBeat % 4 == 0 && ClientPrefs.beatMode == 'Game camera' && ClientPrefs.beatType == '1/16')
		{
			FlxG.camera.zoom += 0.012 * ClientPrefs.camSpeed;
		}
